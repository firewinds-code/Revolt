            ...
            success: function(response) {
                ...
                if (response[0] == 0) {
                    ...
                    for (let row = 0; row < response[1].length; row++) {
                        t.row.add([row + 1, `<i class = "fa fa-edit" style="cursor:pointer" id="searchrowbutton" onclick="getData('${response[1][row].id}','${response[1][row].ticket_no}')"></i>`, response[1][row].ticket_no, response[1][row].customer_name, response[1][row].mobile_number, response[1][row].interaction_type, response[1][row].product_type, response[1][row].contact_reason, response[1][row].contact_reason_type, response[1][row].contact_reason_sub_sub_type, response[1][row].call_status, response[1][row].final_call_status, response[1][row].created_at]).draw(false);
                    }
                    $("#tbl_id").show();
                }
            }
...
<script>
    $("#new_ticket_button").click(function() {
        ...
        $('#dealership_location').append('<option value="' + '">' + ' Select dealership location ' + '</option>');
    ...
    $("#city").change(function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#pincode').append('<option value="' + '">' + ' Select a Pincode ' + '</option>');
                for (val in response) {
                    var newOption = $('<option value="' + response[val]['pincode'] + '">' + response[val]['pincode'] + '</option>');
    ...
    $("#dealership").change(function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#dealership_location').append('<option value="' + '">' + ' Select dealership location ' + '</option>');
                for (val in response) {
                    var newOption = $('<option value="' + response[val]['dealership_location'] + '">' + response[val]['dealership_location'] + '</option>');
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                setTimeout(() => {
                    ...
                    $('#contact_reason_sub_sub').val(response[0][0].contact_reason_sub_sub_type).trigger('change');;
                    $('#pincode').val(response[0][0].pincode);
                    // console.log( $('#dealership_location').val());
                    $('#dealership_location').val(response[0][0].dealership_location);
                ...
                for (let array_no = 0; array_no < response[2].length; array_no++) {
                    table1.row.add([array_no + 1, response[2][array_no].agent_id, response[2][array_no].remark, response[2][array_no].ResolutionProvided, response[2][array_no].ticket_no, response[2][array_no].call_status, response[2][array_no].final_call_status, response[2][array_no].created_at]).draw(false);
                }
                ...
                for (let array_no = 0; array_no < response[1].length; array_no++) {
                    table2.row.add([array_no + 1, response[1][array_no].agent_id, response[1][array_no].remark,
